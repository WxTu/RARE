...
def build_args():
    ...
    parser.add_argument("--loss_r", type=float, default=2.0)
    parser.add_argument("--loss_a", type=float, default=2.0)
    parser.add_argument("--edcoder_rate", type=float, default=0.75)
...
def node_classification_evaluation(model, linear_model, graph, x, num_classes, lr_f, weight_decay_f, max_epoch_f,
                                   device, linear_prob=True, mute=False):
    model.eval()
    with torch.no_grad():
        x = model.embed(graph.to(device), x.to(device))

        in_feat = x.shape[1]
    encoder = copy.deepcopy(linear_model)
    encoder.to(device)
